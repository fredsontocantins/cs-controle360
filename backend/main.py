...
async def get_summary(cycle_id: int | None = None):
    ...
    # Pre-fetch all data once with history to allow in-memory filtering
    ...
    # Pre-calculate datetimes to avoid repeated parsing
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        for activity in atividades_cycle:
            if activity.get("status") != "concluida":
                continue
            # Note: list_atividade already normalizes names
            label = activity.get("executor") or activity.get("owner") or "Sem responsável"
            key = label.casefold()
    ...
    # Calculate main dashboard summary (current cycle only)
    cycle_started_at = get_active_cycle_started_at("reports")
    if cycle_started_at:
        current_activities = _filter_cycle_records(all_activities, cycle_started_at, None, ("created_at", "updated_at", "completed_at"))
        current_homologacoes = _filter_cycle_records(all_homologacoes, cycle_started_at, None, ("check_date", "requested_production_date", "production_date", "created_at"))
        current_customizacoes = _filter_cycle_records(all_customizacoes, cycle_started_at, None, ("received_at", "created_at"))
        current_releases = _filter_cycle_records(all_releases, cycle_started_at, None, ("applies_on", "created_at"))
    else:
        current_activities = []
    ...
    for activity in current_activities:
        if activity.get("status") != "concluida":
            continue
        person_label = activity.get("executor") or activity.get("owner") or "Sem responsável"
