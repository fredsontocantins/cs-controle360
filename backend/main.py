...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # SQL for cycle filtering - handles multi-field COALESCE chain
        h_where = "COALESCE(NULLIF(check_date, ''), NULLIF(requested_production_date, ''), NULLIF(production_date, ''), NULLIF(created_at, '')) >= ? AND (COALESCE(NULLIF(check_date, ''), NULLIF(requested_production_date, ''), NULLIF(production_date, ''), NULLIF(created_at, '')) < ? OR ? IS NULL)"
        c_where = "COALESCE(NULLIF(received_at, ''), NULLIF(created_at, '')) >= ? AND (COALESCE(NULLIF(received_at, ''), NULLIF(created_at, '')) < ? OR ? IS NULL)"
        a_where = "COALESCE(NULLIF(completed_at, ''), NULLIF(updated_at, ''), NULLIF(created_at, '')) >= ? AND (COALESCE(NULLIF(completed_at, ''), NULLIF(updated_at, ''), NULLIF(created_at, '')) < ? OR ? IS NULL)"
        r_where = "COALESCE(NULLIF(applies_on, ''), NULLIF(created_at, '')) >= ? AND (COALESCE(NULLIF(applies_on, ''), NULLIF(created_at, '')) < ? OR ? IS NULL)"

        params = (start_text, end_text, end_text)

        homologacoes = HomologacaoRepository.count(h_where, params)
        customizacoes = CustomizacaoRepository.count(c_where, params)
        atividades_count = AtividadeRepository.count(a_where, params)
        releases = ReleaseRepository.count(r_where, params)

        # Tasks by owner in cycle
        sql_tasks = f"""
            SELECT COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável') as owner_label, COUNT(*) as count
            FROM {AtividadeRepository.table}
            WHERE status = 'concluida' AND {a_where}
            GROUP BY owner_label
            ORDER BY count DESC, owner_label ASC
        """
        rows = run_query(conn, sql_tasks, params).fetchall()
        tasks_by_owner = [{"owner": row[0], "count": row[1]} for row in rows]
    ...
    # Global tasks by owner (all time)
    sql_global_tasks = f"""
        SELECT COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável') as owner_label, COUNT(*) as count
        FROM {AtividadeRepository.table}
        ...
        ORDER BY count DESC, owner_label ASC
    """
    global_rows = run_query(conn, sql_global_tasks).fetchall()
    completed_tasks_by_owner = [{"owner": row[0], "count": row[1]} for row in global_rows]
