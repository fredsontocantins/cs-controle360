...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        def get_range_where(cols: tuple[str, ...]) -> tuple[str, tuple]:
            coalesce_cols = ", ".join([f"NULLIF({c}, '')" for c in cols])
            where = f"COALESCE({coalesce_cols}) >= ?"
            params = [start_text]
            if end_text:
                where += f" AND COALESCE({coalesce_cols}) < ?"
                params.append(end_text)
            return where, tuple(params)

        w_h, p_h = get_range_where(("check_date", "requested_production_date", "production_date", "created_at"))
        homologacoes = HomologacaoRepository.count(w_h, p_h)

        w_c, p_c = get_range_where(("received_at", "created_at"))
        customizacoes = CustomizacaoRepository.count(w_c, p_c)

        w_a, p_a = get_range_where(("created_at", "updated_at", "completed_at"))
        atividades_count = AtividadeRepository.count(w_a, p_a)

        w_r, p_r = get_range_where(("applies_on", "created_at"))
        releases = ReleaseRepository.count(w_r, p_r)

        # Optimized owner aggregation via SQL
        owner_query = f"""
            SELECT
                TRIM(COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável')) as person,
                COUNT(*) as count
        ...
        """
        try:
            cur = run_query(conn, owner_query, p_a)
    ...
    # Optimized overall owner aggregation
    owner_query_all = f"""
        SELECT
            TRIM(COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável')) as person,
            COUNT(*) as count
    ...
    """
    try:
        cur = run_query(conn, owner_query_all)
