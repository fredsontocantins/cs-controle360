...
async def get_summary(cycle_id: int | None = None):
    ...
    # 2. Pre-calculate datetimes for each entity type based on their specific keys
    ...
    for r in all_atividades:
        ...
        # Pre-calculate normalized labels for owner/executor
        r["_owner_label"] = normalize_person_name(r.get("executor") or r.get("owner") or "Sem responsável")
    ...
    # Helper to build summary from pre-fetched data
    ...
    # 3. Build cycle summaries
    previous_cycle_summary = build_cycle_summary(previous_cycle)
    current_cycle_summary = build_cycle_summary(open_cycle)

    # If selected cycle is one of the above, reuse it
    selected_cycle_summary = None
    if cycle_id:
        if previous_cycle and previous_cycle.get("id") == cycle_id:
            selected_cycle_summary = previous_cycle_summary
        elif open_cycle and open_cycle.get("id") == cycle_id:
            selected_cycle_summary = current_cycle_summary
        else:
            selected_cycle_summary = build_cycle_summary(get_cycle(cycle_id))

    # 4. Overall summary (preserving original total count behavior)
