...
async def get_summary(cycle_id: int | None = None):
    ...
    # Pre-fetch all records once to avoid redundant database calls in build_cycle_summary
    ...
    # Filter activities for the main dashboard summary (non-history)
    ...
    summary = {
        "homologacoes": len(_filter_cycle_records(all_homologacoes, cycle_started_at or "", None, ("check_date", "requested_production_date", "production_date", "created_at"))) if cycle_started_at else 0,
        "customizacoes": len(_filter_cycle_records(all_customizacoes, cycle_started_at or "", None, ("received_at", "created_at"))) if cycle_started_at else 0,
        "atividades": len(current_activities),
        "releases": len(_filter_cycle_records(all_releases, cycle_started_at or "", None, ("applies_on", "created_at"))) if cycle_started_at else 0,
