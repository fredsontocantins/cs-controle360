...
async def get_summary(cycle_id: int | None = None):
    ...
    # BOLT OPTIMIZATION:
    # 1. Complexity: Reduced from O(N) to O(1) for item counting by using SQL-level COUNT(*).
    # 2. Memory: Avoids loading thousands of records into Python memory for filtering/grouping.
    # 3. Efficiency: Uses COALESCE in SQL for multi-field date range filtering at the database level.
    # 4. Aggregation: Offloads task grouping by owner to SQL GROUP BY, drastically reducing main thread blocking.
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # SQL-level date range filtering matching Python's any-field logic
        h_where = "(check_date >= ? OR requested_production_date >= ? OR production_date >= ? OR created_at >= ?)"
        h_params = [start_text] * 4
        c_where = "(received_at >= ? OR created_at >= ?)"
        c_params = [start_text] * 2
        a_where = "(completed_at >= ? OR updated_at >= ? OR created_at >= ?)"
        a_params = [start_text] * 3
        r_where = "(applies_on >= ? OR created_at >= ?)"
        r_params = [start_text] * 2

        if end_text:
            h_where += " AND (check_date < ? AND requested_production_date < ? AND production_date < ? AND created_at < ?)"
            h_params.extend([end_text] * 4)
            c_where += " AND (received_at < ? AND created_at < ?)"
            c_params.extend([end_text] * 2)
            a_where += " AND (completed_at < ? AND updated_at < ? AND created_at < ?)"
            a_params.extend([end_text] * 3)
            r_where += " AND (applies_on < ? AND created_at < ?)"
            r_params.extend([end_text] * 2)
        ...
        # Grouping by owner at SQL level - case insensitive using LOWER()
        tasks_query = f"SELECT LOWER(COALESCE(executor, owner, 'Sem responsável')) as person_key, COALESCE(executor, owner, 'Sem responsável') as person, COUNT(*) as count FROM {AtividadeRepository.table} WHERE status = 'concluida' AND {a_where} GROUP BY person_key ORDER BY count DESC, person_key ASC"
        tasks_by_owner = []
        try:
            rows = run_query(conn, tasks_query, tuple(a_params)).fetchall()
            for row in rows:
    ...
    # Global completed tasks grouping via SQL - case insensitive using LOWER()
    global_tasks_query = f"SELECT LOWER(COALESCE(executor, owner, 'Sem responsável')) as person_key, COALESCE(executor, owner, 'Sem responsável') as person, COUNT(*) as count FROM {AtividadeRepository.table} WHERE status = 'concluida' GROUP BY person_key ORDER BY count DESC, person_key ASC"
    completed_tasks_by_owner = []
    try:
        rows = run_query(conn, global_tasks_query).fetchall()
        for row in rows:
