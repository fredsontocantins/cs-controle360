...
async def get_summary(cycle_id: int | None = None):
    ...
    from .models.atividade import AtividadeRepository, get_tasks_by_owner
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # Date filtering logic moved to SQL COALESCE
        # Homologações: check_date, requested_production_date, production_date, created_at
        h_where = "COALESCE(NULLIF(check_date, ''), NULLIF(requested_production_date, ''), NULLIF(production_date, ''), created_at) >= ?"
        h_params = [start_text]
        if end_text:
            h_where += " AND COALESCE(NULLIF(check_date, ''), NULLIF(requested_production_date, ''), NULLIF(production_date, ''), created_at) < ?"
            h_params.append(end_text)

        # Customizações: received_at, created_at
        c_where = "COALESCE(NULLIF(received_at, ''), created_at) >= ?"
        c_params = [start_text]
        if end_text:
            c_where += " AND COALESCE(NULLIF(received_at, ''), created_at) < ?"
            c_params.append(end_text)

        # Atividades: created_at, updated_at, completed_at
        a_where = "COALESCE(NULLIF(completed_at, ''), NULLIF(updated_at, ''), created_at) >= ?"
        a_params = [start_text]
        if end_text:
            a_where += " AND COALESCE(NULLIF(completed_at, ''), NULLIF(updated_at, ''), created_at) < ?"
            a_params.append(end_text)

        # Releases: applies_on, created_at
        r_where = "COALESCE(NULLIF(applies_on, ''), created_at) >= ?"
        r_params = [start_text]
        if end_text:
            r_where += " AND COALESCE(NULLIF(applies_on, ''), created_at) < ?"
            r_params.append(end_text)
        ...
        tasks_by_owner = get_tasks_by_owner(start_text, end_text)
    ...
    # Global counts (standardized across repositories)
    ...
    clientes_total = ClienteRepository.count()
    modulos_total = ModuloRepository.count()

    completed_tasks_by_owner = get_tasks_by_owner()
