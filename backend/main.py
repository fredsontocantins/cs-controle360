...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # SQL-level filtering for cycle summaries
        # Use COALESCE for prioritized date fields
        h_where = "COALESCE(check_date, requested_production_date, production_date, created_at) >= ?"
        h_params = (start_text,)
        if end_text:
            h_where += " AND COALESCE(check_date, requested_production_date, production_date, created_at) < ?"
            h_params += (end_text,)
        homologacoes = HomologacaoRepository.count(h_where, h_params)

        c_where = "COALESCE(received_at, created_at) >= ?"
        c_params = (start_text,)
        if end_text:
            c_where += " AND COALESCE(received_at, created_at) < ?"
            c_params += (end_text,)
        customizacoes = CustomizacaoRepository.count(c_where, c_params)

        a_where = "COALESCE(created_at, updated_at, completed_at) >= ?"
        a_params = (start_text,)
        if end_text:
            a_where += " AND COALESCE(created_at, updated_at, completed_at) < ?"
            a_params += (end_text,)
        atividades_count = AtividadeRepository.count(a_where, a_params)

        r_where = "COALESCE(applies_on, created_at) >= ?"
        r_params = (start_text,)
        if end_text:
            r_where += " AND COALESCE(applies_on, created_at) < ?"
            r_params += (end_text,)
        releases = ReleaseRepository.count(r_where, r_params)

        # SQL-level aggregation for tasks by owner in the cycle
        owner_sql = f"""
            SELECT COALESCE(executor, owner, 'Sem responsável') as label, COUNT(*) as count
            FROM {AtividadeRepository.table}
            WHERE status = 'concluida' AND {a_where}
            GROUP BY COALESCE(executor, owner, 'Sem responsável')
            ORDER BY count DESC, label ASC
        """
        owner_rows = run_query(conn, owner_sql, a_params).fetchall()
    ...
    # Global tasks by owner using SQL aggregation
    global_owner_sql = f"""
        SELECT COALESCE(executor, owner, 'Sem responsável') as label, COUNT(*) as count
        FROM {AtividadeRepository.table}
        ...
        ORDER BY count DESC, label ASC
    """
    global_owner_rows = run_query(conn, global_owner_sql).fetchall()

    completed_tasks_by_owner = []
    for row in global_owner_rows:
        label = normalize_person_name(row[0])
