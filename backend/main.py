...
async def get_summary(cycle_id: int | None = None):
    """Get summary of all entities for dashboard using optimized SQL queries."""
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # Optimized SQL-level aggregation for tasks by owner
        group_where = f"status = 'concluida' AND ({a_where})"
        group_sql = f"""
            SELECT COALESCE(NULLIF(TRIM(executor), ''), NULLIF(TRIM(owner), ''), 'Sem responsável') as person,
                   COUNT(*) as count
            FROM {AtividadeRepository.table}
            WHERE {group_where}
            GROUP BY COALESCE(NULLIF(TRIM(executor), ''), NULLIF(TRIM(owner), ''), 'Sem responsável')
            ORDER BY count DESC, person ASC
        """
        tasks_by_owner = []
        rows = run_query(conn, group_sql, a_params).fetchall()
        for row in rows:
            tasks_by_owner.append({"owner": row[0], "count": row[1]})
    ...
    rows = run_query(conn, group_sql_global).fetchall()
    for row in rows:
        completed_tasks_by_owner.append({"owner": row[0], "count": row[1]})
