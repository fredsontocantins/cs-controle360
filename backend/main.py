...
async def get_summary(cycle_id: int | None = None):
    """Get summary of all entities for dashboard using SQL-level aggregations."""
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # Filter SQL components
        where_parts = ["COALESCE(check_date, requested_production_date, production_date, created_at) >= ?"]
        params = [start_text]
        if end_text:
            where_parts.append("COALESCE(check_date, requested_production_date, production_date, created_at) < ?")
            params.append(end_text)
        homologacoes = HomologacaoRepository.count(" AND ".join(where_parts), tuple(params))

        where_parts = ["COALESCE(received_at, created_at) >= ?"]
        params = [start_text]
        if end_text:
            where_parts.append("COALESCE(received_at, created_at) < ?")
            params.append(end_text)
        customizacoes = CustomizacaoRepository.count(" AND ".join(where_parts), tuple(params))

        where_parts = ["COALESCE(created_at, updated_at, completed_at) >= ?"]
        params = [start_text]
        if end_text:
            where_parts.append("COALESCE(created_at, updated_at, completed_at) < ?")
            params.append(end_text)
        atividades_count = AtividadeRepository.count(" AND ".join(where_parts), tuple(params))

        where_parts = ["COALESCE(applies_on, created_at) >= ?"]
        params = [start_text]
        if end_text:
            where_parts.append("COALESCE(applies_on, created_at) < ?")
            params.append(end_text)
        releases = ReleaseRepository.count(" AND ".join(where_parts), tuple(params))

        # Completed tasks by owner in this cycle
        where_parts = ["status = 'concluida'", "COALESCE(created_at, updated_at, completed_at) >= ?"]
        params = [start_text]
        if end_text:
            where_parts.append("COALESCE(created_at, updated_at, completed_at) < ?")
            params.append(end_text)
        ...
        tasks_by_owner = []
        rows = run_query(conn, sql_tasks, tuple(params)).fetchall()
        for row in rows:
    ...
    completed_tasks_by_owner = []
    rows = run_query(conn, sql_global_tasks).fetchall()
    for row in rows:
