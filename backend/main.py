...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # Build SQL condition for date range filtering using COALESCE to pick the first non-null date
        def date_range_where(fields: tuple[str, ...]) -> tuple[str, tuple]:
        ...
        h_where, h_params = date_range_where(("check_date", "requested_production_date", "production_date", "created_at"))
        homologacoes = HomologacaoRepository.count(where=h_where, params=h_params)

        c_where, c_params = date_range_where(("received_at", "created_at"))
        customizacoes = CustomizacaoRepository.count(where=c_where, params=c_params)

        a_where, a_params = date_range_where(("created_at", "updated_at", "completed_at"))
        atividades_count = AtividadeRepository.count(where=a_where, params=a_params)

        r_where, r_params = date_range_where(("applies_on", "created_at"))
        releases = ReleaseRepository.count(where=r_where, params=r_params)

        # Grouped tasks for the cycle
        tasks_query = f"""
            SELECT LOWER(TRIM(COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável'))) as key,
                   TRIM(COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável')) as person_label,
                   COUNT(*) as count
            FROM {AtividadeRepository.table}
            WHERE status = 'concluida' AND {a_where}
            GROUP BY 1, 2
            ORDER BY count DESC, person_label ASC
        """
        tasks_by_owner = []
        with AtividadeRepository._connect() as conn:
            rows = run_query(conn, tasks_query, a_params).fetchall()
            for row in rows:
                tasks_by_owner.append({"owner": normalize_person_name(row[1]), "count": row[2]})
    ...
    with AtividadeRepository._connect() as conn:
        ...
        for row in rows:
            completed_tasks_by_owner.append({"owner": normalize_person_name(row[1]), "count": row[2]})
