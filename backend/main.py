...
def _filter_cycle_records(records: list[dict], start: str, end: str | None, keys: tuple[str, ...]) -> list[dict]:
    ...
    # Pre-calculate record datetimes to avoid redundant lookups and parsing in loops
    # Optimization: Use a cache key based on the fields we're looking at
    dt_key = f"_dt_{hash(keys)}"

    for record in records:
        if dt_key not in record:
            record_value = _record_datetime(record, keys)
            record[dt_key] = parse_cycle_datetime(record_value) if record_value else None
...
async def get_summary(cycle_id: int | None = None):
    ...
    # Pre-fetch all records once to avoid N+1 queries during cycle summary building
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        for activity in atividades_cycle:
            ...
            # Optimization: Use pre-calculated labels if available
            label = activity.get("_owner_label")
            if not label:
    ...
    for activity in all_activities:
        ...
        # Optimization: Use pre-calculated labels
        person_label = activity.get("_owner_label")
        if not person_label:
            executor = normalize_person_name(activity.get("executor"))
            owner = normalize_person_name(activity.get("owner"))
            person_label = executor or owner or "Sem responsável"
            activity["_owner_label"] = person_label
