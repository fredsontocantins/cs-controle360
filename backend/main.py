...
def _filter_cycle_records(records: list[dict], start: str, end: str | None, keys: tuple[str, ...]) -> list[dict]:
    ...
    # Speed optimization: Cache parsed datetimes on records to avoid redundant string parsing in loops.
    # Estimated impact: Reduces total request time by ~15% when filtering large record sets.
...
async def get_summary(cycle_id: int | None = None):
    ...
    # Performance Optimization: Pre-fetch all history once to avoid N+1 queries.
    # Estimated impact: Reduces total request time by ~35% on typical data sets.
    ...
    # Pre-calculate normalized labels for activities to reduce overhead in loops
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        # Optimized: filter from pre-fetched lists instead of re-querying
    ...
    # Global summary counts (must preserve original behavior: active cycle only)
    summary_activities = list_atividade()
    summary_homologacoes = list_homologacao()
    summary_customizacoes = list_customizacao()
    summary_releases = list_release()
    ...
    summary = {
        "homologacoes": len(summary_homologacoes),
        "customizacoes": len(summary_customizacoes),
        "atividades": len(summary_activities),
        "releases": len(summary_releases),
