...
def _filter_cycle_records(records: list[dict], start: str, end: str | None, keys: tuple[str, ...]) -> list[dict]:
    ...
    for record in records:
        # Check for pre-calculated _dt to skip parsing
...
async def get_summary(cycle_id: int | None = None):
    ...
    # Pre-fetch all records once to avoid N+1 queries in build_cycle_summary
    ...
    # Pre-calculate metadata for faster filtering and grouping
    ...
    for r in all_atividades:
        ...
        r["_owner_label"] = normalize_person_name(r.get("executor")) or normalize_person_name(r.get("owner")) or "Sem responsável"
    for r in all_releases:
        r["_dt"] = parse_cycle_datetime(_record_datetime(r, ("applies_on", "created_at")))

    # Pre-parse cycle dates for sorting and windowing
    for cycle in cycles:
    ...
    def get_window(target_cycle: dict) -> tuple[datetime, datetime | None]:
        ...
        later = [c for c in cycles if c["id"] != target_cycle["id"] and c["_dt"] > start]
        later.sort(key=lambda c: c["_dt"])
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        homologacoes_count = len(_filter_cycle_records(all_homologacoes, start_text or "", end_text, ()))
        customizacoes_count = len(_filter_cycle_records(all_customizacoes, start_text or "", end_text, ()))
        atividades_cycle = _filter_cycle_records(all_atividades, start_text or "", end_text, ())
        releases_count = len(_filter_cycle_records(all_releases, start_text or "", end_text, ()))
    ...
    # Current cycle data for top-level counts (to match original behavior)
    curr_start = open_cycle["_dt"] if open_cycle else datetime.min

    current_homologacoes = [r for r in all_homologacoes if r["_dt"] >= curr_start]
    current_customizacoes = [r for r in all_customizacoes if r["_dt"] >= curr_start]
    current_atividades = [r for r in all_atividades if r["_dt"] >= curr_start]
    current_releases = [r for r in all_releases if r["_dt"] >= curr_start]

    completed_tasks_by_owner: list[dict[str, object]] = []
    grouped: dict[str, dict[str, object]] = {}
    for activity in current_atividades:
        if activity.get("status") != "concluida":
            continue
