...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        if not start or start <= parse_cycle_datetime(None):
            return {
        ...
        def get_cycle_where(cols: tuple[str, ...], start_val: str, end_val: str | None) -> tuple[str, tuple]:
            # Replicates: _record_datetime(entity, keys)
            # COALESCE(NULLIF(col1, ''), NULLIF(col2, ''), ...)
            coalesce_expr = f"COALESCE({', '.join([f'NULLIF({c}, \'\')' for c in cols])})"
            where = f"{coalesce_expr} >= ?"
            ps = [start_val]
            if end_val:
                where = f"({where}) AND ({coalesce_expr} < ?)"
                ps.append(end_val)
            return where, tuple(ps)

        h_where, h_params = get_cycle_where(("check_date", "requested_production_date", "production_date", "created_at"), start_text, end_text)
        homologacoes = HomologacaoRepository.count(h_where, h_params)

        c_where, c_params = get_cycle_where(("received_at", "created_at"), start_text, end_text)
        customizacoes = CustomizacaoRepository.count(c_where, c_params)

        a_where, a_params = get_cycle_where(("created_at", "updated_at", "completed_at"), start_text, end_text)
        ...
        r_where, r_params = get_cycle_where(("applies_on", "created_at"), start_text, end_text)
    ...
    completed_tasks_by_owner = AtividadeRepository.get_tasks_by_owner()
