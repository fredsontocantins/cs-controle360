...
def _filter_cycle_records(records: list[dict], start: str | datetime, end: str | datetime | None, keys: tuple[str, ...]) -> list[dict]:
...
async def get_summary(cycle_id: int | None = None):
    ...
    for r in all_activities:
        for k in ("created_at", "updated_at", "completed_at"):
            if k in r and r[k]: r[f"_dt_{k}"] = parse_cycle_datetime(r[k])
        # Pre-normalize names
        r["_norm_executor"] = normalize_person_name(r.get("executor"))
        r["_norm_owner"] = normalize_person_name(r.get("owner"))
