...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        ...
        def _get_where_dates(fields: tuple[str, ...], start: str, end: str | None) -> tuple[str, tuple]:
            coalesce = f"COALESCE({', '.join(fields)})"
            if end:
                return f"{coalesce} >= ? AND {coalesce} < ?", (start, end)
            return f"{coalesce} >= ?", (start,)

        h_where, h_params = _get_where_dates(("check_date", "requested_production_date", "production_date", "created_at"), start_text, end_text)
        homologacoes = HomologacaoRepository.count(h_where, h_params)

        c_where, c_params = _get_where_dates(("received_at", "created_at"), start_text, end_text)
        customizacoes = CustomizacaoRepository.count(c_where, c_params)

        a_where, a_params = _get_where_dates(("created_at", "updated_at", "completed_at"), start_text, end_text)
        atividades_count = AtividadeRepository.count(a_where, a_params)

        r_where, r_params = _get_where_dates(("applies_on", "created_at"), start_text, end_text)
        releases = ReleaseRepository.count(r_where, r_params)

        # SQL for completed tasks by owner in cycle
        sql_tasks = f"""
            SELECT COALESCE(NULLIF(executor, ''), NULLIF(owner, ''), 'Sem responsável') as person, COUNT(*) as count
            FROM {AtividadeRepository.table}
            WHERE status = 'concluida' AND {_get_where_dates(('created_at', 'updated_at', 'completed_at'), start_text, end_text)[0]}
            GROUP BY person
        ...
        try:
            with AtividadeRepository._connect() as conn_cycle:
                rows = run_query(conn_cycle, sql_tasks, a_params).fetchall()
    ...
    """
    if DATABASE_URL:
        sql_global_tasks = sql_global_tasks.replace("?", "%s")

    completed_tasks_by_owner: list[dict[str, object]] = []
    try:
        with AtividadeRepository._connect() as conn_tasks:
            rows = run_query(conn_tasks, sql_global_tasks).fetchall()
            completed_tasks_by_owner = [{"owner": row[0], "count": row[1]} for row in rows]
    except Exception:
        completed_tasks_by_owner = []
