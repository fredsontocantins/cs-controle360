...
async def get_summary(cycle_id: int | None = None):
    ...
    def build_cycle_summary(cycle: dict | None) -> dict[str, object] | None:
        if not cycle:
            return None
        start_dt, end_dt = get_cycle_window(cycle["id"])
        ...
        t_where = a_where + " AND status = 'concluida'"
        tasks_by_owner = AtividadeRepository.get_tasks_by_owner(t_where, tuple(a_params))
