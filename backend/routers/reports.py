...
async def get_consolidated_intelligence(
    ...
    # ⚡ Bolt Optimization: Pre-fetch operational records once and reuse when cycle_id is None
    all_atividades = atividade.list_atividade(include_history=cycle_id is not None)
    all_releases = release_model.list_release(include_history=cycle_id is not None)
    ...
    playbook_dashboard = playbook_gen.build_dashboard(playbooks, all_atividades, all_releases)
