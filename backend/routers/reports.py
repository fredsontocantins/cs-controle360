...
async def get_consolidated_intelligence(
    ...
    # 1. PDF Intelligence with pre-fetched documents
    pdf_service = PDFIntelligenceService()
    docs = list_documents()
    pdf_context = pdf_service.refresh_application_context(docs=docs)
    pdf_audit = pdf_service.build_cycle_audit(cycle_id=cycle_id, docs=docs)
    ...
    # 3. Playbook dashboard - reuse pre-fetched activities and releases when cycle_id is None
