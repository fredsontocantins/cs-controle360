...
class ReportService:
    ...
    def get_summary_text(self, **kwargs):
        clean_kwargs = {**kwargs}
        clean_kwargs.pop("release_name", None)
        clean_kwargs.pop("cycle_started_at", None)
        pdf_context = self.pdf_service.refresh_application_context()
        release_id = clean_kwargs.get("release_id")
        activities = self._get_activities(release_id)
        release_name = self._resolve_release_name(release_id, None)
        ...
        return self.generator.generate_summary_report(
            ...
            pdf_context=pdf_context,
            release_name=release_name,
            cycle_started_at=self._resolve_cycle_started_at(clean_kwargs.get("cycle_id")),
            **clean_kwargs
        )

    def get_html_report(self, **kwargs):
        clean_kwargs = {**kwargs}
        req_release_name = clean_kwargs.pop("release_name", None)
        clean_kwargs.pop("cycle_started_at", None)
        pdf_context = self.pdf_service.refresh_application_context()
        release_id = clean_kwargs.get("release_id")
        activities = self._get_activities(release_id)
        release_name = self._resolve_release_name(release_id, req_release_name)
