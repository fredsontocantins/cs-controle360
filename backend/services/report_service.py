...
class ReportService:
    ...
    def get_summary_text(self, **kwargs):
        kwargs_clean = {k: v for k, v in kwargs.items() if k not in ("release_name", "cycle_started_at")}
        pdf_context = self.pdf_service.refresh_application_context()
        ...
        return self.generator.generate_summary_report(
            ...
            pdf_context=pdf_context,
            release_name=release_name,
            cycle_started_at=self._resolve_cycle_started_at(kwargs_clean.get("cycle_id")),
            **kwargs_clean
        )
