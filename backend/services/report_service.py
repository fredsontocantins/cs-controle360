...
class ReportService:
    ...
    def get_summary_text(self, **kwargs):
        gen_kwargs = {**kwargs}
        pdf_context = self.pdf_service.refresh_application_context()
        release_id = gen_kwargs.get("release_id")
        activities = self._get_activities(release_id)
        release_name = self._resolve_release_name(release_id, gen_kwargs.pop("release_name", None))
        cycle_started_at = self._resolve_cycle_started_at(gen_kwargs.get("cycle_id"))
        gen_kwargs.pop("cycle_started_at", None)
