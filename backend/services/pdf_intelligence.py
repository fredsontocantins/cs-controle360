...
class PDFIntelligenceService:
    ...
    def build_cycle_audit(self, cycle_id: Optional[int] = None) -> Dict[str, Any]:
        """Return audit counts and cycle information for PDF documents."""
        ...
        error = [d for d in docs if d.get("analysis_state") == "error"]
        return {
            "cycle": cycle,
            "counts": {
                "total": len(docs),
    ...
    def process_documents(
        ...
        """Process specified staged PDF documents or pending documents in active cycle."""
        docs = list_documents()
        if document_ids:
        ...
        for d in target_docs:
            full_path = UPLOADS_DIR / Path(d.get("pdf_path", "")).name
            if not full_path.exists():
                skipped.append(d)
                messages.append(f"Arquivo não encontrado: {d.get('filename')}")
                continue
            try:
