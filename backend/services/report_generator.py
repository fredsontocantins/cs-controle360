...
class ReportGenerator:
    ...
    def _build_management_report(
        ...
        # Bolt performance optimization: Pre-fetch operational lists once
        # to eliminate 8 redundant database queries inside cycle summary calculations.
        homologacoes_all = list_homologacao(include_history=True)
        customizacoes_all = list_customizacao(include_history=True)
        atividades_all = list_atividade(include_history=True)
        releases_all = list_release(include_history=True)

        homologacoes = homologacoes_all
        customizacoes = customizacoes_all
        if cycle_start:
            homologacoes = [
                row
        ...
        if open_cycle:
            ...
            if current_start > datetime.min:
                ...
                current_cycle_summary = {
                    ...
                    "homologacoes": _count_in_window(homologacoes_all, current_start, current_end, ("check_date", "requested_production_date", "production_date", "created_at")),
                    "customizacoes": _count_in_window(customizacoes_all, current_start, current_end, ("received_at", "created_at")),
                    "atividades": _count_in_window(atividades_all, current_start, current_end, ("created_at", "updated_at", "completed_at")),
                    "releases": _count_in_window(releases_all, current_start, current_end, ("applies_on", "created_at")),
                }
        if previous_cycle:
            ...
            if previous_start > datetime.min:
                previous_cycle_summary = {
                    ...
                    "homologacoes": _count_in_window(homologacoes_all, previous_start, previous_end, ("check_date", "requested_production_date", "production_date", "created_at")),
                    "customizacoes": _count_in_window(customizacoes_all, previous_start, previous_end, ("received_at", "created_at")),
                    "atividades": _count_in_window(atividades_all, previous_start, previous_end, ("created_at", "updated_at", "completed_at")),
                    "releases": _count_in_window(releases_all, previous_start, previous_end, ("applies_on", "created_at")),
                }
