...
def ensure_tables() -> None:
    ...
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_AUTH_AUDIT} (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, actor_user_id INTEGER, actor_username TEXT, target_user_id INTEGER, target_username TEXT, event_type TEXT, action TEXT NOT NULL, provider TEXT, message TEXT, ip_address TEXT, user_agent TEXT, details_json TEXT, status TEXT, created_at TEXT NOT NULL)")
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_REPORT_CYCLE} (id INTEGER PRIMARY KEY AUTOINCREMENT, scope_type TEXT NOT NULL, scope_id INTEGER, scope_label TEXT, cycle_number INTEGER NOT NULL, period_label TEXT, status TEXT DEFAULT 'aberto', notes TEXT, opened_at TEXT, closed_at TEXT, created_at TEXT NOT NULL, updated_at TEXT)")
