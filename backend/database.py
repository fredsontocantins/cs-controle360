...
def ensure_tables() -> None:
    ...
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_USER} (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, full_name TEXT, role TEXT DEFAULT 'user', is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, email TEXT, provider TEXT DEFAULT 'local', google_sub TEXT, approval_status TEXT DEFAULT 'approved', last_login_at TEXT)")
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_AUTH_AUDIT} (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, action TEXT NOT NULL DEFAULT 'audit', message TEXT, ip_address TEXT, user_agent TEXT, status TEXT, created_at TEXT NOT NULL, actor_user_id INTEGER, actor_username TEXT, target_user_id INTEGER, target_username TEXT, event_type TEXT, provider TEXT, details_json TEXT)")
