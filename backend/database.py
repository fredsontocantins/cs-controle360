...
def ensure_tables() -> None:
    ...
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_USER} (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, email TEXT, password_hash TEXT, full_name TEXT, role TEXT DEFAULT 'user', provider TEXT DEFAULT 'local', google_sub TEXT, approval_status TEXT DEFAULT 'pending', is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, last_login_at TEXT)")
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_AUTH_AUDIT} (id INTEGER PRIMARY KEY AUTOINCREMENT, actor_user_id INTEGER, actor_username TEXT, target_user_id INTEGER, target_username TEXT, event_type TEXT, provider TEXT, status TEXT, message TEXT, details_json TEXT, created_at TEXT NOT NULL)")
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_REPORT_CYCLE} (id INTEGER PRIMARY KEY AUTOINCREMENT, scope_type TEXT NOT NULL, scope_id INTEGER, scope_label TEXT, cycle_number INTEGER NOT NULL, period_label TEXT, status TEXT DEFAULT 'aberto', notes TEXT, created_at TEXT NOT NULL, updated_at TEXT, closed_at TEXT)")
