...
def ensure_tables() -> None:
    ...
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_USER} (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, email TEXT, password_hash TEXT NOT NULL, role TEXT DEFAULT 'user', provider TEXT DEFAULT 'local', google_sub TEXT, full_name TEXT, approval_status TEXT DEFAULT 'approved', is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, last_login_at TEXT)")
    conn.execute(f"CREATE TABLE IF NOT EXISTS {TABLE_AUTH_AUDIT} (id INTEGER PRIMARY KEY AUTOINCREMENT, actor_user_id INTEGER, actor_username TEXT, target_user_id INTEGER, target_username TEXT, event_type TEXT, user_id INTEGER, username TEXT, action TEXT, message TEXT, ip_address TEXT, user_agent TEXT, status TEXT, provider TEXT, details_json TEXT, created_at TEXT NOT NULL)")

    # Ensure added columns exist if table already existed
    for col, defn in [
        ("email", "TEXT"),
        ...
        _ensure_column(conn, TABLE_USER, col, defn)
    ...
    for col, defn in [
        ...
        _ensure_column(conn, TABLE_AUTH_AUDIT, col, defn)
