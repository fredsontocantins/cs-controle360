...
def parse_cycle_datetime(value: Any) -> datetime:
    """Parse cycle datetime with performance optimization for ISO format."""
    if not value:
        return datetime.min

    text = str(value).strip()

    # Try ISO format first as it is significantly faster (fromisoformat)
    try:
    ...
    for fmt in (
        ...
        try:
            # Performance: slice only when necessary for %S formats with potential extra chars
