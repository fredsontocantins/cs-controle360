...
def parse_cycle_datetime(value: Any) -> datetime:
    """
    Parse a date/time value with caching and optimized format detection.
    Performance: Prioritizes fromisoformat and uses LRU cache to minimize overhead.
    """
    if not value:
        return datetime.min
    ...
    for fmt in (
        ...
        try:
            # Handle the legacy truncation logic for formats ending in %S if needed
