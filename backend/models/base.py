...
class BaseRepository:
    ...
    def list(cls, where: str = "", params: tuple = ()) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            sql = f"SELECT * FROM {cls.table}{where_clause} ORDER BY {cls.order_by}"
            with cls._connect() as conn:
                if DATABASE_URL:
                    with conn.cursor(cursor_factory=RealDictCursor) as cur:
                        # Convert ? to %s for Postgres if needed, though run_query does it
                        # BaseRepository methods should probably use run_query but they
                        # currently use raw connection/cursor for specialized cursor types.
                        query = sql.replace("?", "%s")
    ...
    def count(cls, where: str = "", params: tuple = ()) -> int:
        ...
        try:
            where_clause = f" WHERE {where}" if where else ""
            sql = f"SELECT COUNT(*) FROM {cls.table}{where_clause}"
            with cls._connect() as conn:
                cursor = run_query(conn, sql, params)
                row = cursor.fetchone()
