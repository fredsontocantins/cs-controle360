...
def insert_auth_audit(data: Dict[str, Any]) -> int:
    # Map service-layer fields to SQLite schema
    payload = {
        "user_id": data.get("actor_user_id") or data.get("target_user_id"),
        "username": data.get("actor_username") or data.get("target_username"),
        "action": f"{data.get('event_type', 'unknown')}:{data.get('provider', 'local')}",
