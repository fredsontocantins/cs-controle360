...
export async function GET() {
  ...
  // Fetch counts and activity details in a single batch of parallel queries
  const [
    { count: homologacoes },
    { count: customizacoes },
    { data: activities, count: atividades },
    { count: releases },
    { count: clientes },
    { count: modulos },
  ] = await Promise.all([
  ...
  // Aggregate status and owner counts in a single O(N) pass
